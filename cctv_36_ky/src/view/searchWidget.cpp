    ...
    , userManager(new UserManager(this))
{
    ...
    connect(ui->pushButton_enroll, &QPushButton::clicked, this, &SearchWidget::clicked_buttonEnroll);
    connect(ui->pushButton_edit, &QPushButton::clicked, this, &SearchWidget::clicked_buttonEdit);
    //connect(ui->pushButton_delete, &QPushButton::clicked, this, &SearchWidget::clicked_buttonDelete);
...
void SearchWidget::setupTable() {
    ...
    /* Debugging: Print existing all data(Table:Basic) to Terminal
...
{
    // 검색 버튼 클릭 시 검색 수행
    connect(ui->searchButton, &QPushButton::clicked, this, &SearchWidget::performSearch);

    // Enter 키 입력 시 검색 수행
    connect(ui->searchInput, &QLineEdit::returnPressed, this, &SearchWidget::performSearch);

    // 텍스트 변경 시 실시간 검색
    connect(ui->searchInput, &QLineEdit::textChanged, this, &SearchWidget::handleSearchInput);

    // 테이블 뷰에 있는 컬럼 더블 클릭시 이벤트 연결
    connect(ui->resultsTable, &QTableView::doubleClicked, this, &SearchWidget::handleDoubleClick);

    // 검색어 필터링
    connect(ui->filterButton, &QPushButton::clicked, this, &SearchWidget::showSearchMenu);
}

void SearchWidget::handleSearchInput(const QString &text)
{
    // 실시간 검색을 위한 로직
...
void SearchWidget::handleDoubleClick(const QModelIndex &index) {
...
bool SearchWidget::initializePath() {
...
void SearchWidget::updatePlaceholder()
{
...
/* KIYUN_1127 */
QString SearchWidget::get_seletedData() {
...
void SearchWidget::refreshTable() {
    setupTable();
}

void SearchWidget::lineEdit_test() {
    ui->lineEdit_test->setText("hi");
}

void SearchWidget::clicked_buttonEnroll() {
    EnrollDialog *enrollDialog = new EnrollDialog(this);
    connect(enrollDialog, &EnrollDialog::dataSubmitted, this, &SearchWidget::refreshTable);
